...
/**
 * KINTEL — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'Kammandor-Intel/1.0', 'Accept': 'application/json' },
    });

    console.log('[KINTEL] IODA response status:', res.status);
    ...
    console.error('[KINTEL] IODA fetch error:', error);
