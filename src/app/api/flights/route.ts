...
async function getOpenSkyToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.warn('[KAMMANDOR] OpenSky token failed:', res.status); return null; }
    const data = await res.json();
    if (!data.access_token) {
      console.warn('[KAMMANDOR] OpenSky token response missing access_token');
      return null;
    }
    ...
    console.warn('[KAMMANDOR] OpenSky token error:', e);
    return null;
  }
}
...
export async function GET() {
  ...
  fetchPromise = (async () => {
    ...
    if (osRes.status === 'fulfilled') {
      if (osRes.value.status === 429) {
        ...
        console.warn('[KAMMANDOR] OpenSky 429 — cooling down 15 min');
        ...
        try {
          ...
          console.warn('[KAMMANDOR] OpenSky parse error:', e);
        }
      } else {
        await osRes.value.body?.cancel();
      }
    ...
    if (!openSkyWorked) {
      source = 'regional';
      console.warn('[KAMMANDOR] OpenSky unavailable — fanning out 30×2 regional queries');
  ...
  try {
    ...
    console.error('[KAMMANDOR] Flight fetch error:', error);
    fetchPromise = null;
    return NextResponse.json({ error: 'Failed to fetch flight data' }, { status: 500 });
  }
}
