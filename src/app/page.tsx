...
const ZuluClock = () => {
  ...
  return <span className="text-[var(--gold-primary)] font-bold tabular-nums">{time || 'ZULU --:--:--Z'}</span>;
};
...
export default function Dashboard() {
  ...
  const [appTheme, setAppTheme] = useState<'core'|'ghost'>('ghost');
  ...
  // Splash screen
  useEffect(() => {
    // In embed mode skip splash immediately for a clean embedded experience
    const delay = embedMode ? 0 : 2500;
    const splashTimer = setTimeout(() => setShowSplash(false), delay);
  ...
  useEffect(() => {
    ...
    // ── Parse all launch context params ──
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[KINTEL] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  useEffect(() => {
    (window as any).openKintelIntel = (entity: any) => {
    ...
    return () => { delete (window as any).openKintelIntel; };
  }, []);
  ...
  const fetchEndpoint = useCallback(async (url: string, transform?: (d: any) => any, options?: RequestInit) => {
    ...
    try {
      ...
      console.warn('[KINTEL] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  // ── Kammandor SDK — Intelligence Fusion Layer ──
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
          ...
          >
            ...
            <div className="relative w-40 h-40 mb-8 flex items-center justify-center z-[2]">
              ...
              <motion.div
                ...
                style={{ inset: '18px', border: '1px solid rgba(232,160,32,0.15)' }}
              >
                <div className="absolute top-1/2 right-0 translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full" style={{ background: 'var(--gold-primary)', boxShadow: '0 0 10px var(--gold-primary), 0 0 20px rgba(232,160,32,0.2)' }} />
                <div className="absolute bottom-0 left-1/4 translate-y-1/2 w-1 h-1 rounded-full" style={{ background: 'rgba(232,160,32,0.4)' }} />
              </motion.div>
            ...
            {/* ── Kammandor title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'KAMMANDOR'.split('').map((letter, i) => (
                <motion.span
                  key={i}
            ...
            <div className="w-64 md:w-80 z-[2]">
              ...
              <div className="relative w-full h-[2px] rounded-full overflow-hidden" style={{ background: 'rgba(212,175,55,0.1)' }}>
                <motion.div
                  ...
                  style={{ background: 'linear-gradient(90deg, var(--gold-deep), var(--gold), var(--gold-light))', boxShadow: '0 0 12px rgba(232,160,32,0.4)' }}
                />
              </div>
              ...
              <div className="mt-3 h-4 flex items-center justify-center">
                {[
                  ...
                  <motion.span
                    ...
                    style={{ color: i === 3 ? 'var(--gold)' : 'var(--text-muted)' }}
                  >
                    {stage.text}
                  </motion.span>
                ))}
      ...
      >
        ...
        >
          {mapProjection === 'globe' ? (
            ...
            <Globe className="w-5 h-5 text-[var(--gold-primary)] group-hover:scale-110 transition-transform" />
          )}
        ...
        >
          {mapStyle === 'dark' ? (
            ...
            <Moon className="w-5 h-5 text-[var(--gold-primary)] group-hover:scale-110 transition-transform" />
          )}
          <span className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 text-[9px] font-mono text-[var(--text-muted)] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity glass-panel px-2 py-1 z-[300]">
      ...
      {!embedMode && <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          ...
          <div className="flex flex-col items-start gap-0.5">
            ...
            <span className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] uppercase" style={{ color: 'var(--gold)', opacity: 0.8 }}>LIVE INVESTMENT INTELLIGENCE</span>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[5px] md:text-[6px] text-[var(--text-muted)] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate">
            KAMMANDOR INTEL · LIVE INVESTMENT INTELLIGENCE <span className="hidden md:inline">· DEALS · COUNTERPARTIES · SANCTIONS · MARKETS</span>
          </span>
        </div>
      </motion.div>}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-4 text-[9px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <span className="hidden lg:inline-flex items-center gap-1">
          <span className="text-[var(--gold-primary)] font-bold">{Object.values(activeLayers).filter(Boolean).length}</span>
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={appTheme} setTheme={setAppTheme} />}
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        <div className="relative group">
          <button onClick={() => { setShowIntel(!showIntel); setShowMarkets(false); setShowAlerts(false); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${showIntel ? 'bg-[var(--gold-primary)]/20' : 'hover:bg-white/10'}`}>
          ...
          <AnimatePresence>
            {showIntel && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="absolute right-12 top-1/2 -translate-y-1/2 w-80">
                <OsintPanel theme={appTheme} setTheme={setAppTheme} onSweepVisualize={setSweepData} onScanGeolocate={(target, data) => {
                  setScanTargets(prev => {
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-kintel-pulse" />
              ...
              {liveFeedEmbedAllowed ? (
                ...
                <div className="w-full aspect-video flex items-center justify-center bg-black/95">
                  <div className="text-center px-8">
                    <div className="w-14 h-14 rounded-full bg-[var(--gold-primary)]/10 border border-[var(--gold-primary)]/20 flex items-center justify-center mx-auto mb-4">
                      <ExternalLink className="w-6 h-6 text-[var(--gold-primary)]" />
                    </div>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-6 py-2.5 rounded border border-[var(--gold-primary)]/40 text-[var(--gold-primary)] font-mono text-[12px] hover:bg-[var(--gold-primary)]/10 transition-colors tracking-wider"
                    >
            ...
            <div className="glass-panel mobile-nav-inner">
              {[
                ...
                <button key={tab.id} onClick={() => setMobilePanel(mobilePanel === tab.id ? null : tab.id)}
                  ...
                  <tab.icon className={`w-4 h-4 ${tab.id === 'recon' ? 'text-[var(--gold-primary)]' : ''}`} />
                  <span className={tab.id === 'recon' ? 'text-[var(--gold-primary)]' : ''}>{tab.label}</span>
                </button>
              ))}
            </div>
          </div>
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[9px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'RECON TOOLKIT' : 'SEARCH'}
                    </span>
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} isMobile={true} theme={appTheme} setTheme={setAppTheme} />
                      <div className="mt-8">
      ...
      {!isMobile && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3, duration: 0.8 }} className="desktop-only absolute bottom-8 z-[200] pointer-events-auto" style={{ left: '72px' }}>
          <div className="flex items-center gap-6 text-[8px] font-mono tracking-widest text-[var(--text-muted)] opacity-60">
            ...
            <div className="flex gap-2 items-center">
              <span>LOC</span>
              <span className="text-[var(--gold-primary)] truncate max-w-[200px]">{locationLabel || 'HOVER MAP'}</span>
            </div>
            <div className="flex gap-2 items-center">
              <span>Z</span>
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 kintel-glow">
