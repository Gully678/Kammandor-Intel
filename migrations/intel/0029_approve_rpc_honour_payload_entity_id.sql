-- 0029_approve_rpc_honour_payload_entity_id.sql
-- Mission A (populate the ontology): create_entity now honours an OPTIONAL
-- client-supplied payload `id` (uuid). Why: mappers propose entities AND the
-- links between them in one governed batch; intel.link FKs to intel.entity, so
-- link proposals can only bind if the approved entity keeps the id the mapper
-- referenced. Absent/empty payload id => gen_random_uuid() (behaviour of
-- 0012/0014 unchanged for every existing mapper). A duplicate id fails LOUDLY
-- on the primary key -- never a silent overwrite.
--
-- Supersedes the FUNCTION BODY of intel.approve_proposed_edit from 0014 only.
...
-- Only the create_entity INSERT differs from 0014 (id column + coalesce).
