-- 0031_platform_super_admin_cross_tenant_review.sql
-- Mission A unblock: the approve/reject review RPCs denied ALL cross-tenant
-- callers unconditionally — including the platform operator. INVRT's
-- super_admin (organizations.is_platform = true) therefore could not approve
-- ANY tenant's proposals, and tenants with no users (e.g. new clients) had no
-- possible approver at all.
--
-- Change (both functions, tenant-match block ONLY): when the caller's org is
-- NOT the edit's tenant, the call is now allowed ONLY IF the caller is a
-- super_admin AND their organisation has is_platform = true (checked against
-- public.organizations, never against client input). Every other cross-tenant
-- caller — including a super_admin of a NON-platform org — remains hard-denied.
-- Same-tenant behaviour is byte-identical to 0029/0015.
--
-- Impersonation-proven before apply (rolled-back transaction):
--   [P] platform super_admin claims -> cross-tenant approve SUCCEEDS
--   [N1] tenant 'guest' claims      -> approve DENIED (role not approver)
--   [N2] non-platform-org super_admin claims -> cross-tenant approve DENIED
-- Re-runnable proof: tests/rls/cross_tenant_review.sql
--
-- GOVERNANCE BOUNDARY unchanged and re-stated: intel.approve_proposed_edit()
-- remains the SOLE governed writer to intel.entity / intel.link /
-- intel.entity_provenance. SECURITY DEFINER; ALL authorisation enforced
-- in-body against the caller's JWT claims. Do NOT relax further.
...
begin
  ...
  if v_caller_org <> v_edit.tenant_id then
    -- 0031: ONLY a super_admin of the platform organisation may act cross-tenant.
    if not (
  ...
  if v_edit.kind = 'create_entity' then
    ...
    insert into intel.entity (
      id, tenant_id, type, canonical_name, properties, risk_score, risk_category,
      last_screened_at, lei, company_number, imo, mmsi, isin, wallet_address,
      jurisdiction_code
    )
    select
      coalesce(nullif(v_edit.payload ->> 'id', '')::uuid, gen_random_uuid()),
      v_edit.tenant_id,
      v_edit.payload ->> 'type',
...
comment on function intel.approve_proposed_edit(uuid) is
  ...
  'Since 0029, create_entity honours an optional payload id (uuid) so batch '
  'link proposals can bind to their sibling entities; duplicates fail loudly. '
  'Since 0031, a super_admin of the platform organisation (is_platform = true) '
  'may review cross-tenant; all other cross-tenant callers are hard-denied.';
...
-- ---------------------------------------------------------------------------
-- reject_proposed_edit — same cross-tenant rule (body otherwise 0015-identical)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_caller_org <> v_edit.tenant_id then
    ...
    if not (
      ...
      raise exception 'reject_proposed_edit: denied — caller organisation does not match proposed_edit tenant' using errcode='insufficient_privilege';
    end if;
  end if;
